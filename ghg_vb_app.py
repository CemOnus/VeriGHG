...
# ghg_vb_app.py (patched LOG mode)
import os
...
import pandas as pd
import numpy as np
import io, re, math, json, datetime as dt
...
st.set_page_config(page_title='GHG VB Prototype (Patched)', layout='wide')
...
def standardize_and_quantify(records):
    ...
    for r in records:
        ...
        rows.append({
            'source': r['source'], 'category': cat,
            'scope': scope, 'activity_value': val, 'unit_hint': unit_hint,
...
# Sidebar
st.sidebar.title('GHG VB Prototype (Patched)')
...
st.title('GHG Verification Body – Prototype (Patched LOG)')
        ...
        if f.name.lower().endswith('.csv'):
            ...
            with st.expander(f'Parsed PDF: {f.name}'):
                ...
                if parsed['lines']:
                    ...
                    st.session_state.records.extend([r for r in extract_from_text_lines(parsed['lines'], source_name)])
        else:
...
st.caption('Patched build with duplicate-column handling (LOG mode).')
